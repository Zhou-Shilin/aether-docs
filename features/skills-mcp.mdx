...
To automatically activate a Skill for every new chat, enable it in the Skill picker and tap **Set as Default**. Aether stores your selection in `default_skill_ids` in the app state and applies it to all new conversations going forward. You can still override defaults per chat.
...
Use the **Skill picker** in the composer to enable or disable individual Skills per chat. The `default_skill_ids` field in the app state governs which Skills are auto-selected when you open a brand new chat. You can update defaults at any time without affecting existing conversations.
