...
<ParamField path="aether.extensions" type="string[]">
  Array of paths to Aether Script Mod entry files, relative to the package root. Each file is loaded by the Aether Script Mod runtime (API v2). Export your factory as `activateAether` or `aether` (or as `default` for an Aether-only package) to register surfaces, components, pages, actions, and event handlers. Changes to these files hot-reload without restarting the app. Omit this field if your extension does not use the Script Mod API.
</ParamField>
...
When you share a single entry file between `pi.extensions` and `aether.extensions`, keep the Pi factory as the `default` export and export the Aether factory as `activateAether` or `aether`. The Script Mod runtime looks for `activateAether` or `aether` first; it falls back to `default` only for packages listed exclusively under `aether.extensions`.
