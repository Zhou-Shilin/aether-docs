...
Imported extension packages live under `~/.aether/extensions` inside Aether's managed Alpine filesystem. When you import a zip via Settings → Extensions → **Import extension**, Aether extracts the package into a subdirectory there. Packages installed through Aether's package management are discovered through the Pi package manager.
