...
Use `pi.registerTool` to expose a callable function to the LLM. Provide a TypeBox `parameters` schema so the model knows what arguments to supply, and return an `AgentToolResult` from `execute`.
