...
- **Storage writes trigger surface re-renders** — `storage.set()`, `storage.delete()`, and `storage.clear()` invalidate extension UI automatically. In this mod, `toggle-expanded` updates `storage.expanded` and the framework re-renders the tray without an explicit `aether.invalidate()` call.
