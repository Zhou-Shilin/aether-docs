...
Aether extensions can subscribe to app events and intercept operations to observe, modify, or cancel built-in behaviors. Events let you react to app activity; interceptors let you actively shape or stop operations before they complete.
...
`on()` registers a handler for a named event and returns a cleanup function you can call to unsubscribe. Event names are defined by Aether core; use `intercept()` to act on the built-in operations documented below.
...
- **Return `{ payload: {...} }`** — merges the supplied fields into the chained payload.
- **Return `{ cancel: true, reason: "..." }`** — stops the operation entirely and surfaces the reason.

## `chat.new` example

Add a Skill to every new chat by intercepting `chat.new` and returning the updated `selected_skill_ids`:
