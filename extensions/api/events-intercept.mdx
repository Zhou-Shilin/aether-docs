...
`on()` registers a handler for a named event and returns a cleanup function you can call to unsubscribe. Common events include `before_send`, which fires before a chat message is submitted.

```typescript
aether.on("before_send", ({ draft_input }) => {
  console.log("User is sending:", draft_input);
});
```
