...
<ParamField path="id" type="string" required>
  Unique page identifier. Aether uses this to scope the page within your extension and to target the page from `ui.node("pageButton", { page: pageId })` navigation calls. Must be a non-empty string.
</ParamField>
...
<ParamField path="render" type="function">
  A factory function that returns the page's `ui` node tree. The function may be synchronous or `async` and receives the standard render context. Use either `render` or `tree`.
...
Use `ui.node("pageButton", { page, label, icon })` in any surface, component, or other page to render a navigation button that opens the target page:
