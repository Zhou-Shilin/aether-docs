...
<ParamField path="aether.api" type="number | object">
  Script API 兼容性。可使用精确整数 `2`，或 `{ "min": 2, "max": 2, "allowNewer": false }` 形式的对象。为兼容旧包，缺少声明时仍允许加载。不兼容或格式错误的声明会拒绝重载，并保留之前可用的 Script 运行时。
</ParamField>

<ParamField path="aether.native.api" type="number | object">
  Native API 兼容性。当前 Native API 版本为 `1`。可使用精确整数，或与 `aether.api` 相同的 `min`、`max` 及可选 `allowNewer` 范围格式。
</ParamField>
...
导入的扩展包位于 Aether 管理的 Alpine 文件系统中的 `~/.aether/extensions`。通过 设置 → 扩展 → **导入扩展** 导入 zip 时，Aether 会先暂存包，再替换现有副本。包内已有 `node_modules` 时会直接使用；否则，有运行时依赖且存在锁文件时执行 `npm ci --omit=dev`，没有锁文件时执行 `npm install --omit=dev`。依赖安装或重载失败时会恢复之前的包。通过 Aether 包管理安装的包由 Pi 包管理器发现。
...
<Note>
  `defineAetherExtension` 辅助函数为 `aether` 参数提供 TypeScript 类型，运行时是空操作 — 它只是原样返回你传入的工厂函数。从 `@baimoqilin/aether-extension-api` 导入即可。
</Note>
