...
| `SCHEDULE_EXACT_ALARM` | Improves the timing accuracy of scheduled and time-triggered agent tasks |
| `USE_EXACT_ALARM` | Allows Aether to use exact alarms where Android grants that capability |
| `RECEIVE_BOOT_COMPLETED` | Reschedules pending tasks automatically after your device restarts |
| `REQUEST_INSTALL_PACKAGES` | Allows Aether to install downloaded app updates through the Android package installer |
...
| `moe.shizuku.manager.permission.API_V23` | **Optional.** Required to authorize Agent Mode's isolated virtual display through Shizuku |
...
Install Termux from [F-Droid](https://f-droid.org/en/packages/com.termux/) (recommended) or [GitHub Releases](https://github.com/termux/termux-app/releases). The Play Store version does not provide the `RUN_COMMAND` integration Aether requires.

### Shizuku

[Shizuku](https://shizuku.rikka.app/) lets apps use Android system APIs with elevated privileges without requiring a fully rooted device. Enabling Shizuku integration in Aether authorizes Agent Mode to create and control an isolated Android virtual display.
